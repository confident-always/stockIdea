...
def compute_dmi(df: pd.DataFrame, period: int = 39) -> pd.DataFrame:
    """
    计算DMI指标（按照自定义指标算法）
    返回包含PDI、MDI、ADX的DataFrame
    
    算法公式：
    TR1 := EMA(MAX(MAX(HIGH-LOW,ABS(HIGH-REF(CLOSE,1))),ABS(LOW-REF(CLOSE,1))),N)
    HD := HIGH-REF(HIGH,1)
    LD := REF(LOW,1)-LOW
    ...
    PDI: DMP*100/TR1
    MDI: DMM*100/TR1
    ADX: EMA(ABS(MDI-PDI)/(MDI+PDI)*100,N)
    """
    if len(df) < period + 1:
        return df.assign(PDI=np.nan, MDI=np.nan, ADX=np.nan)
    ...
    tr_component1 = high - low
    tr_component2 = (high - prev_close).abs()
    tr_component3 = (low - prev_close).abs()
    
    # 取三个分量的最大值
    tr_raw = pd.concat([tr_component1, tr_component2, tr_component3], axis=1).max(axis=1)
    
    # 使用EMA平滑TR1
    alpha = 2.0 / (period + 1)
    tr1 = tr_raw.ewm(alpha=alpha, adjust=False).mean()
    ...
    # 使用EMA平滑DMP和DMM
    dmp = dmp_raw.ewm(alpha=alpha, adjust=False).mean()
    dmm = dmm_raw.ewm(alpha=alpha, adjust=False).mean()
    ...
    # 计算ADX
    # ADX: EMA(ABS(MDI-PDI)/(MDI+PDI)*100,N)
    adx_raw = (mdi - pdi).abs() / (mdi + pdi) * 100
    adx = adx_raw.ewm(alpha=alpha, adjust=False).mean()

    # 统一保留三位小数（用于算法与输出的一致性）
    pdi = pdi.round(3)
    mdi = mdi.round(3)
    adx = adx.round(3)
    
    return df.assign(PDI=pdi, MDI=mdi, ADX=adx)
...
class ADXSelector:
    ...
    def _find_recent_adx_cross_index(self, dmi_data: pd.DataFrame) -> Optional[int]:
        ...
        for i in range(start, n):
            ...
            if (
                pd.isna(prev_adx) or pd.isna(prev_mdi) or pd.isna(curr_adx) or pd.isna(curr_mdi)
                or pd.isna(pdi.iloc[i])
            ):
                continue
    ...
    def _post_cross_price_ok(self, hist: pd.DataFrame, cross_idx: int) -> bool:
        ...
        # cross_open = float(hist['open'].iloc[cross_idx])
        # if not np.isfinite(cross_open):
        #     return False

        # if cross_idx + 1 >= len(hist):
        #     # 上穿发生在最后一天，无后续交易日，视为满足
        #     return True

        # after_close = hist['close'].iloc[cross_idx + 1 :]
        # if after_close.empty:
        #     return True

        # return (after_close >= cross_open).all()
        return True
    
    def _passes_filters(self, hist: pd.DataFrame) -> bool:
        """
        简化的过滤条件：检查“ADX上穿MDI”且满足后续收盘价约束
        """
        if len(hist) < self.dmi_period + 1:
            return False
        ...
        # 寻找最近发生的“ADX 上穿 MDI”索引（限定在 lookback_days 内）
        cross_idx = self._find_recent_adx_cross_index(dmi_data)
        if cross_idx is None:
            return False
