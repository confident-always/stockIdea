...
> **更新时间：2025-09-10** –
>
> 1. 重构 `fetch_kline.py`：仅使用 **Tushare 日线（不复权）**、从 **`stocklist.csv`** 读取股票池、支持排除板块（创业板/科创板/北交所），抓取为**全量覆盖保存**；
...
### 运行select_stock.py
```bash
python select_stock.py --data-dir ./data --config ./configs.json --date 2025-09-30 --meta-workers 8 --adx-periods 39,105,243
```
---
...
* **DMI 指标计算**：使用 39 周期计算 DMI 指标（ADX、PDI、MDI），采用 EMA 平滑算法；
* **时间维度检查**：在指定的 lookback_days 天数内，检查 ADX 和 MDI 的差值：
  - 任何一天 |ADX - MDI| ≤ 1.0 即满足条件
  - 不再要求 ADX 必须上穿 MDI，而是关注两者的接近程度
...
├── fetch_kline.py           # 从 stocklist.csv 读取并抓取 Tushare 日线（不复权）
├── fetch_kline_from_ipo.py  # 按每只股票上市日→至今抓取不复权日线
