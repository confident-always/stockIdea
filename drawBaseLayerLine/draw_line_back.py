...
class BackLineDrawer:
    ...
    def generate_B_series_back(self, A: float, N: float, k_list: List[int], 
        ...
        for k in k_list:
            B_k = A + N * k
            if B_k <= max_price * 1.2:  # 只生成不超过最高价120%的线
                B_values.append(B_k)
                K_values.append(k)
    ...
    def compute_anchor_back_lines(self, df: pd.DataFrame, anchor_idx: int, anchor_date, 
        ...
        try:
            ...
            if best_N is None:
                if N_results:
                    max_matches = max(r['matches_count'] for r in N_results.values())
                    logger.info(f"⚠️ [{stock_code}] 未找到满足条件的N值(要求>={min_matches}个匹配，实际最多{max_matches}个)，跳过AnchorBack线")
                else:
                    logger.info(f"⚠️ [{stock_code}] 未找到满足条件的N值，跳过AnchorBack线")
                return None
