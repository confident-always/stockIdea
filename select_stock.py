...
def main():
    ...
    for cfg in selector_cfgs:
        ...
        # 在选股前打印 ADX 参数信息（若为 ADXSelector）
        if cfg.get("class") == "ADXSelector":
            lb = getattr(selector, "lookback_days", None)
            dp = getattr(selector, "dmi_period", None)
            logger.info("")
            logger.info("【参数提示】ADX战法：DMI周期=%s；lookback_days=%s", dp, lb)
            if isinstance(lb, int) and lb > 0:
                logger.info("这是最近 %d 天范围内 ADX 上穿 MDI 的数据", lb)
            else:
                logger.info("这是最近若干天范围内 ADX 上穿 MDI 的数据")

            # 若为 ADXSelector，则按指定/默认的多个周期分别生成 CSV：ADX{period}_YYYY-MM-DD.csv
            periods: List[int]
            if args.adx_periods:
                try:
                    periods = [int(x.strip()) for x in args.adx_periods.split(",") if x.strip()]
                except Exception:
                    logger.warning("解析 --adx-periods 失败，使用默认 [39, 105, 243]")
                    periods = [39, 105, 243]
            else:
                periods = [39, 105, 243]

            # 为每个周期单独实例化 ADXSelector 并输出对应 CSV
            try:
                module = importlib.import_module("Selector")
            ...
            for period in periods:
                ...
                if hasattr(selector_p, "select_with_details"):
                    try:
                        ...
                        logger.error("ADX%d 详情选股失败：%s", period, e)
                        detailed_picks = []
                else:
                    ...
                    try:
                        ...
                        logger.error("ADX%d 选股失败：%s", period, e)
                        detailed_picks = []

                # 打印当期结果
                alias_p = f"ADX{period}"
                logger.info("")
                logger.info("============== 选股结果 [%s] ==============", alias_p)
                ...
                # 生成 CSV：仅包含 code, date, ADX
                out_dir = Path("res")
                out_dir.mkdir(exist_ok=True)
                out_file = out_dir / f"{alias_p}_{trade_date.date().isoformat()}.csv"
                try:
                    ...
                    for r in detailed_picks:
                        ...
                        try:
                            if compute_dmi and code in data:
                                ...
                                dmi_df = compute_dmi(hist, period)
                                last_adx = dmi_df["ADX"].iloc[-1]
                                if pd.notna(last_adx):
                                    adx_val = round(float(last_adx), 3)
                        except Exception as e:
                            logger.warning("计算最后一日 ADX 失败 %s(%d)：%s", code, period, e)
                    ...
                    logger.error("生成 ADX%d CSV 失败: %s", period, e)

            # 已针对 ADXSelector 完成多周期导出，跳过后续通用写入逻辑
            continue
